/* ================================================================
   ...
   Same data as git-log-v2 (cleaned up, tightened), rendered as
   a four-column log pane (rail / hash / date / message) with
   expandable `git show` panels. No text-splatter. Approachable.
   ================================================================ */
...
window.TIMELINE = (function () {
  ...
  const BRANCHES = [
    ...
    { id: 'oscar',       name: 'oscar-health', colorVar: '--c-oscar',       years: '20172021' },
    { id: 'youni',       name: 'youni',        colorVar: '--c-youni',       years: '20152016' },
    { id: 'commercehub', name: 'commercehub',  colorVar: '--c-commercehub', years: '20132016' },
    { id: 'rubiks',      name: 'rubiks-cube',  colorVar: '--c-rubiks',      years: '20082014' },
    { id: 'married',     name: 'married',      colorVar: '--c-married',     years: '20252026' },
  ];
  ...
  const COMMITS = [
    ...
    },
    {
      ...
      body:
        "Competed 20082014 at Northeast US and Nationals. 3x3 average: 13.95 seconds. " +
  ...
  function render(bodyEl) {
    ...
    for (const c of COMMITS) {
      ...
      out.push(`<div class="${classes.join(' ')}" ${colorStyle} data-sha="${c.sha}" data-branch="${c.branch}">
        ...
        <div class="tl-expand">
          <button class="tl-close" data-close>close</button>
  ...
  function renderBranchFilter(containerEl) {
    containerEl.innerHTML = '';
    // Filter chips are embedded in the right rail; nothing else needed.
  ...
  function wire(bodyEl, opts) {
    ...
    bodyEl.addEventListener('click', (e) => {
      ...
      if (!row) return;
      // only toggle if not clicking a link/button inside
      if (e.target.closest('a')) return;
      const isOpen = row.classList.contains('open');
      // collapse others for cleanliness (single-expanded)
      bodyEl.querySelectorAll('.tl-commit.open').forEach((r) => { if (r !== row) r.classList.remove('open'); });
      row.classList.toggle('open', !isOpen);
      if (!isOpen) {
        // scroll into view a bit
        const top = row.getBoundingClientRect().top;
        if (top < 120 || top > window.innerHeight - 200) {
          row.scrollIntoView({ block: 'start', behavior: 'smooth' });
        }
      }
    });

    // branch filter (in right rail)
  ...
  return {
    ...
    render, renderBranchRail, renderJumpRail, renderBranchFilter,
    wire, rawSource,
  };
})();
