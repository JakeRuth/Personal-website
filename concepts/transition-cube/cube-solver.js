/* ================================================================
   transition-cube — 3x3 solver
   ================================================================

   Adapted from concepts/xp-luna-v2/cube-solver.js (bidirectional BFS on
   full cubie state). Same representation, same move semantics.

   Exposed surface (window.TransitionCubeSolver):
   ...
   Representation:
     Corners: 8 positions (URF, UFL, ULB, UBR, DFR, DLF, DBL, DRB)
       - cp[0..7] = which corner cubie is at each position
       - co[0..7] = orientation (0,1,2)
     Edges: 12 positions (UF, UL, UB, UR, FL, BL, BR, FR, DF, DL, DB, DR)
       - ep[0..11] = which edge cubie is at each position
       - eo[0..11] = orientation (0,1)

   Moves: quarter turns only (face + optional "'" suffix). The transition
   animation never needs doubles; the bidirectional BFS handles any state
   reachable within maxPerSide*2 quarter turns. We keep scrambles at <= 8
   moves to guarantee a fast, always-findable solve (well inside budget).
   ================================================================ */
...
(function () {
  ...
  // Bidirectional BFS on full cube state.
  ...
  // ================================================================
  // Facelet model (for rendering).
  //   Face order: U R F D L B.
  //   Sticker grid per face:
  //     0 1 2
  //     3 4 5
  //     6 7 8
  // ================================================================
  ...
  function faceletsToState(fc) {
    ...
    for (let pos = 0; pos < 8; pos++) {
      ...
      const primarySlot = colors.indexOf(primaryColor);
