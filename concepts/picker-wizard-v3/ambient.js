/* =========================================================
   ...
   Carried over from picker-wizard-v2 unchanged (Jake likes it).
   ========================================================= */
(function () {
  "use strict";
  ...
  function makeCubie(x, y, z) {
    ...
    // Six materials in order: +x, -x, +y, -y, +z, -z.
    const materials = [
  ...
  // --- "Tracks": each cube follows a slow parametric curve in 3D. ---
  // Tracks are invisible; we just sample them to position cubes. The
  // result is gentle, arcing drift - never straight lines, never chaotic.
  //
  // Track shape: lissajous-ish. Different phases/scales per cube so they
  // never visibly sync up.
  function makeTrack(seed) {
    const rnd = mulberry32(seed);
    const ax = 7 + rnd() * 4;     // x amplitude
    const ay = 3 + rnd() * 2.5;   // y amplitude
    const az = 5 + rnd() * 3;     // z amplitude
    const px = rnd() * Math.PI * 2;
    const py = rnd() * Math.PI * 2;
    const pz = rnd() * Math.PI * 2;
    const fx = 0.11 + rnd() * 0.08;   // frequency (slow!)
    const fy = 0.07 + rnd() * 0.06;
    const fz = 0.09 + rnd() * 0.07;
    // Base depth offset: push each cube to a different z-plane so
    // there's always a mix of near and far cubes on screen.
  ...
  for (let i = 0; i < N_CUBES; i++) {
    ...
    // Each cube gets its own slow spin axis + rate.
    ...
    // Random starting orientation so none look "aligned to screen."
  ...
  // --- Opacity veil ---
  // The whole scene is rendered at reduced opacity via canvas CSS so it
  // sits behind the wizard gracefully. We could do per-material opacity,
  // but a canvas-level opacity is cheaper and looks identical at this
  // softness.
  ...
  if (reduceMotion) {
    // Position cubes at t=0 and draw once.
    for (let i = 0; i < cubes.length; i++) {
      const c = cubes[i];
      const p = c.track(0);
      c.mesh.position.set(p.x, p.y, p.z);
